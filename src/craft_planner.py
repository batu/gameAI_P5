...
def graph(current_node, all_states):
    ...
    for r in all_recipes:
        if r.check(state):
            ...
            # When we make it so it takes it into consideration the performance hit was too big to
...
def heuristic(current_state, child_node, goal, queue): #take goal here.
    ...
    #This heuristic invalidates the usage of lower technology items if you have a higher tech one
    # Eg: Dont punch trees if you have an axe lying around
    if invalidate_low_tech(current_state, action_name):
        return inf

    #For loop is for a possible extension where recipes create more than a single product.
...
#{} with effect {}".format(product, current_state[product], child_node.effect.recipe_name))
        ...
        # This section takes that into consideration.
        if product == "cobble":
...
def search(graph, state, is_goal, limit, heuristic, goal):
        ...
        while time() - start_time < limit and queue:
            ...
            if is_goal(current_state):
                ...
                print("The search algorithm explored {} states.".format(num_states))
                return path
...
if __name__ == '__main__':
    ...
    resulting_plan = search(graph, init_state, is_goal, 30, heuristic, Crafting['Goal'])
